...
// Use a loop to generate and add options to the select element
...
function handleSearchResult(resultDataString) {
    ...
    if (resultDataJson["status"] === "success") {
        window.location.replace('school_list.html?school=' + resultDataJson['school'] + "&location=" + resultDataJson['location'] + "&other=" + resultDataJson['other'] + "&order=" + resultDataJson['order'] + "&genre=" + resultDataJson['genre']);
    } else {
...
function handleBrowseResult(resultDataString) {
    const selectElement = document.querySelector('#browse_way');

    selectElement.addEventListener('change', (event) => {
        const selectedValue = event.target.value;
        if(selectedValue=="genres"){
            window.location.replace('school_list.html?school='+ "&location="+  "&other="+"&order="+ "&genre="+ "public university");
        }
        else{
            window.location.replace('school_list.html?school='+ "&location="+  "&other="+"&order="+"ASC"+ "&genre=");
        }
